...
/** Active storefront marketing offers (sorted: featured first). */
export function getStorefrontOffers() {
...
/** GET /api/orders/payment-summary/:orderId — requires secret token (query). */
export async function getPaymentOrderSummary(orderId, token) {
  const q = new URLSearchParams({ token }).toString();
  return fetchJson(`/api/orders/payment-summary/${orderId}?${q}`);
}

/** POST /api/payments/dummy/complete — server verifies token + updates order. */
export async function completeDummyPayment({ orderId, paymentToken, outcome }) {
  return fetchJson('/api/payments/dummy/complete', {
    method: 'POST',
    body: JSON.stringify({ orderId, paymentToken, outcome }),
  });
}
