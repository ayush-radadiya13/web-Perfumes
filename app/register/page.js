...
function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0f0e14] p-4">
      ...
      <div className="hidden lg:flex lg:w-[45%] relative rounded-l-2xl overflow-hidden">
        ...
        <div className="absolute top-6 left-6 z-20">
          <Link href="/" className="text-white font-display text-xl tracking-wide">
            Lumière
          </Link>
        </div>
        <Link
        ...
        <div className="absolute bottom-8 left-6 right-6 z-20 text-white">
          <p className="text-lg font-display">Capturing Moments, Creating Memories</p>
        </div>
      </div>

      {/* Right panel - form */}
      <div className="flex-1 flex items-center justify-center p-6 sm:p-10 lg:p-14 overflow-y-auto">
        <div className="w-full max-w-md">
          <div className="lg:hidden mb-8 flex justify-between items-center">
            <Link href="/" className="text-white font-display text-xl">
              Lumière
            </Link>
            <Link
              href="/"
              className="px-4 py-2 rounded-full bg-white/10 text-white/90 text-sm"
            >
              Back to website →
            </Link>
          </div>
          ...
          <form onSubmit={onSubmit} className="space-y-5">
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 text-cream-muted hover:text-cream p-1"
            ...
            <div>
              <span className="block text-sm text-cream-muted mb-3">Gender</span>
            ...
            <label className="flex items-start gap-3 cursor-pointer text-cream-muted text-sm">
              <input
                type="checkbox"
                name="terms"
                required
                className="mt-1 w-4 h-4 rounded border-white/30 bg-white/5 text-purple focus:ring-2 focus:ring-purple/40 accent-purple"
              />
              <span>
                I agree to the{' '}
